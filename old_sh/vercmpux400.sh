...
{
	echo ""
	echo -e "version compare tool for veex net-updater systm(v300), v$version"
	echo ""
	echo -e "Usage:"
...
#get version number
verold=$(cat /tmp/vercmp/old/usr/local/lib/vendor-info.txt | grep ".*-Release-.*" | gawk '{print $2}')
vernew=$(cat /tmp/vercmp/new/usr/local/lib/vendor-info.txt | grep ".*-Release-.*" | gawk '{print $2}')
...
compare_dir()
{
	...
	pushd /tmp/vercmp/old/$1 > /dev/null
	...
	#compare files
	while read fname; do file_checknew "$1/$fname" "$1"; done < /tmp/vercmp/files


	#find deleted files
	while read fname; do file_checkrm "$1/" "$fname" ; done < /tmp/vercmp/oldfiles

	mv /tmp/vercmp/diff/rm.txt /tmp/vercmp/diff/rm-$1.txt  2>/dev/null

	echo "Compress .."
	cd /tmp/vercmp/diff/$1/$1
	err_check "Enter /tmp/vercmp/diff/$1/$1 failed"

	tar cpf /tmp/vercmp/diff/$1.tar *
	err_check "compress to $1.tar.gz failed"

	rm -rf /tmp/vercmp/diff/$1
