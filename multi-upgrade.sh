...
#		echo -e "Unsupport USB device" > ${display_dev}
	...
	if [ $update -eq 0 ]; then
		if [ $sdup -eq 0 ]; then
			...
			echo -n "SD card need upgrade" > ${display_dev}
		fi
	fi
...
#	cp -rf /usr/bin/mdarmsd /resv/v400/
	...
	if [ "$usbdev" == "$USB0" ]; then
		...
		sleep 5
		/usr/bin/client 128.0.0.2 $udport 128.0.0.1 $imagename $boardname /resv/v400/usb0fup.log > ${display_dev}
	  	...
	  	if [ $? -ne 0 ]; then
			...
			return 3
				...
				rm -rf /resv/v400/usb0sdup.log
	                        sleep ${DELAY_SD}
				/usr/bin/client 128.0.0.2 $udport 128.0.0.1 ux400-module-sd.tar.gz $boardname /resv/v400/usb0sdup.log > ${display_dev}
                	        if [ $? -ne 0 ]; then
		...
		sleep 5
		/usr/bin/client 128.0.0.6 $udport 128.0.0.5 $imagename $boardname /resv/v400/usb1fup.log > ${display_dev}
	  	...
	  	if [ $? -ne 0 ]; then
			...
			/sbin/ux400cset
				...
				rm -rf /resv/v400/usb1sdup.log
                                sleep ${DELAY_SD}
                                /usr/bin/client 128.0.0.6 $udport 128.0.0.5 ux400-module-sd.tar.gz $boardname /resv/v400/usb1sdup.log > ${display_dev}
                                if [ $? -ne 0 ]; then
					/usr/bin/sdupfail > ${display_dev}
					sleep 1
					halt
		...
		sleep 5
		/usr/bin/client 128.0.0.10 $udport 128.0.0.9 $imagename $boardname /resv/v400/usb2fup.log > ${display_dev}
	  	...
	  	if [ $? -ne 0 ]; then
			...
			return 3
				...
				rm -rf /resv/v400/usb2sdup.log
                                sleep ${DELAY_SD}
                                /usr/bin/client 128.0.0.10 $udport 128.0.0.9 ux400-module-sd.tar.gz $boardname /resv/v400/usb2sdup.log > ${display_dev}
                                if [ $? -ne 0 ]; then
					/usr/bin/sdupfail > ${display_dev}
					sleep 1
					halt
		...
		sleep 5
		/usr/bin/client 128.0.0.14 $udport 128.0.0.13 $imagename $boardname /resv/v400/usb3fup.log > ${display_dev}
		...
		if [ $? -ne 0 ]; then
			...
			return 3
				...
				rm -rf /resv/v400/usb3sdup.log
                                sleep ${DELAY_SD}
                                /usr/bin/client 128.0.0.14 $udport 128.0.0.13 ux400-module-sd.tar.gz $boardname /resv/v400/usb3sdup.log > ${display_dev}
                                if [ $? -ne 0 ]; then
					/usr/bin/sdupfail > ${display_dev}
					sleep 1
					halt
		...
		sleep 5
		/usr/bin/client 128.0.0.18 $udport 128.0.0.17 $imagename $bardname /resv/v400/usb4fup.log > ${display_dev}
	  	...
	  	if [ $? -ne 0 ]; then
			...
			return 3
				...
				rm -rf /resv/v400/usb4sdup.log
                                echo "Flash upgrade done, find module SD card image, upgrade SD card." > ${display_dev}
                                sleep ${DELAY_SD}
                                /usr/bin/client 128.0.0.18 $udport 128.0.0.17 ux400-module-sd.tar.gz $boardname /resv/v400/usb4sdup.log > ${display_dev}
                                if [ $? -ne 0 ]; then
					/usr/bin/sdupfail > ${display_dev}
					sleep 1
					halt
		...
		sleep 5
		/usr/bin/client 128.0.0.22 $udport 128.0.0.21 $imagename $boardname /resv/v400/usb5fup.log > ${display_dev}
	  	...
	  	if [ $? -ne 0 ]; then
			...
			return 3
				...
				rm -rf /resv/v400/usb5sdup.log
                                sleep ${DELAY_SD}
                                /usr/bin/client 128.0.0.22 $udport 128.0.0.21 ux400-module-sd.tar.gz $boardname /resv/v400/usb5sdup.log > ${display_dev} 
                                if [ $? -ne 0 ]; then
					/usr/bin/sdupfail > ${display_dev}
					sleep 1
					halt
...
function multiup()
{
	mtype $1
	machine="$?"
	echo "xxha:----------- upgrade slot $1 start" > ${display_dev}
	if [ "$machine" == "$UNKNOW" ]; then
	...
	echo "xxha:----------- upgrade slot $1 done" > ${display_dev}
}
